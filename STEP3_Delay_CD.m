...
%%% Delay CD analysis within and across imaging sessions
% Trial number >= 50
...
savefnn = strcat('mouse',num2str(mouse_id),'_FOV',num2str(FOV),'_case_',case_id,'_delay.mat');
...
% define Delay CD time window (4 bins)
epoch_bin = floor([1.57 4.17 4.37]*sr);
...
for tt=1:size(sessions,1)
    ...
    for zzt = 1:rep_no
        ...
        disp(strcat('Estimating Delay CD, round=',num2str(zzt),'/',num2str(rep_no)))
    ...
    for ttt=1:size(sessions,1)
        disp(strcat('Projecting onto estimated Delay CD, train:',num2str(tt),',test:',num2str(ttt)))
        for zzt = 1:rep_no
...
i_mode = 1;     % Delay CD for primary axis
...
for zzt = 1:rep_no
    for tt=1:no_se
        ...
        for zz=1:no_se
            ...
            % CD delay classifier performance using delay dF/F0
...
% final decoding performance of Delay CD
...
disp('Plotting single trials projected onto Delay CD')
...
hold on
sgtitle('Delay CD - deconvolved')

save(savefnn,'cell_no','rep_no','decoder_d_d','CD_proj_d_test',...
    'dot_all','axis_all','orthonormal_basis_d')
