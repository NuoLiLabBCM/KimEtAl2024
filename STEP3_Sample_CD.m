...
%%% Sample CD analysis within and across imaging sessions
% Trial number >= 50
...
savefnn = strcat('mouse',num2str(mouse_id),'_FOV',num2str(FOV),'_case_',case_id,'_sample.mat');
...
% define Sample CD time window (5 bins)
...
for tt=1:size(sessions,1)
    ...
    for zzt = 1:rep_no
        ...
        disp(strcat('Estimating Sample CD, round=',num2str(zzt),'/',num2str(rep_no)))
    ...
    for ttt=1:size(sessions,1)
        disp(strcat('Projecting onto estimated Sample CD, train:',num2str(tt),',test:',num2str(ttt)))
        for zzt = 1:rep_no
...
i_mode = 1;     % Sample CD for primary axis
...
for zzt = 1:rep_no
    for tt=1:no_se
        ...
        % mean of lick right/left correct points on CD sample projection
        ...
        % variance of lick right/left correct points on CD sample projections
        ...
        for zz=1:no_se
            ...
            % CD sample classifier performance using sample dF/F0
...
% final decoding performance of Sample CD
...
disp('Plotting single trials projected onto Sample CD')
...
save(savefnn,'cell_no','rep_no','decoder_s_s','CD_proj_s_test',...
    'dot_all','axis_all','orthonormal_basis_s')
