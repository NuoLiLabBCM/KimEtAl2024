...
FOV = 1;
case_id = 'oxoxx';
savefn = strcat('mouse',num2str(mouse_id),'_FOV',num2str(FOV),'_case_',case_id,'OR.mat');
...
for tt=1:size(sessions,1)
    ...
    for i=1:size(plane_all,1)
        ...
        ROI_sessions{tt,1}{6,1}(i,:) = mean(plane_all_decon_re{i,1},1);     % all trials
        ...
        ROI_sessions{tt,1}{6,2}(i,:) = mean(plane_all_decon_re{i,2},1);     % all trials
        ...
        ROI_sessions{tt,1}{6,3}(i,:) = mean(plane_all_decon_re{i,3},1);     % all trials
        ...
        ROI_sessions{tt,1}{6,4}(i,:) = mean(plane_all_decon_re{i,4},1);     % all trials
...
clear epoch epoch2    
epoch(1,:) = round([1.57 3.5 4.27]*6);
epoch2(1,:) = round([2.3 4.17 5.27]*6);
